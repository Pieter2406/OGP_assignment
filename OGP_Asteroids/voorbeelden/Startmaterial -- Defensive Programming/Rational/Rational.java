...
public class Rational {
	...
	public boolean isIdenticalTo(Rational other)
		...
		try {
			...
			// It is good practice to add assert statements to catchers, clarifying
	...
	public Rational times(long factor) throws TimesOverflowException {
		try {
			try {
				...
				// efficient computations for small rational numbers and factors.
				...
				// number (which must be a normalized rational number at this point),
				// have some factor in common, we invoke the method again reversing
				// the roles of the factor and the numerator of this rational number.
				Rational toReduce = new Rational(factor, this.getDenominator()).normalize();
	...
	public Rational add(Rational other) throws OverflowException,
		...
		try {
			long crossProduct1 = ExtMath.times(getNumerator(), other.getDenominator());
			long crossProduct2 = ExtMath.times(getDenominator(), other.getNumerator());
			long resultNumerator = ExtMath.add(crossProduct1, crossProduct2);
			long resultDenominator = ExtMath
